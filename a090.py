...
def myai(board: List[List[int]]) -> Tuple[int, int] | None:
    """
    エントリー関数（提出用）
    board: 8x8, 1=黒, -1=白, 0=空
    戻り値: (r, c) / 置けないなら None（パス）
    """
    # ここで深さを調整してOK（重いなら 3、強くしたいなら 5）
    return choose_best_move(board, current_player=1, depth=4)
